...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None,  plot_S='line', plot_E='line', plot_I='line',plot_R='line', plot_F='line',
        ...
        if(any(Fseries) and plot_F=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Fseries<=0, self.tseries), numpy.ma.masked_where(Fseries<=0, topstack+Fseries), topstack, color=color_F, alpha=0.5, label='$Fatality$', zorder=2)
        ...
        if(any(Eseries) and plot_E=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Eseries<=0, self.tseries), numpy.ma.masked_where(Eseries<=0, topstack+Eseries), topstack, color=color_E, alpha=0.5, label='$Exposed$', zorder=2)
        ...
        if(combine_D and plot_D_E=='stacked' and plot_D_I=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, topstack+Dseries), topstack, color=color_D_E, alpha=0.5, label='$Detected$', zorder=2)
        ...
        if(any(Iseries) and plot_I=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Iseries<=0, self.tseries), numpy.ma.masked_where(Iseries<=0, topstack+Iseries), topstack, color=color_I, alpha=0.5, label='$Infected$', zorder=2)
        ...
        if(any(Fseries) and plot_F=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Fseries<=0, self.tseries), numpy.ma.masked_where(Fseries<=0, Fseries), 0, color=color_F, alpha=0.5, label='$Fatality$', zorder=4)
        ...
        if(any(Eseries) and plot_E=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Eseries<=0, self.tseries), numpy.ma.masked_where(Eseries<=0, Eseries), 0, color=color_E, alpha=0.5, label='$Exposed$', zorder=4)
        ...
        if(combine_D and (any(Dseries) and plot_D_E=='shaded' and plot_D_E=='shaded')):
            ax.fill_between(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, Dseries), 0, color=color_D_E, alpha=0.5, label='$Detected$', zorder=4)
        ...
        if(any(Iseries) and plot_I=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Iseries<=0, self.tseries), numpy.ma.masked_where(Iseries<=0, Iseries), 0, color=color_I, alpha=0.5, label='$Infected$', zorder=4)
        ...
        if(any(Fseries) and plot_F=='line'):
            ax.plot(numpy.ma.masked_where(Fseries<=0, self.tseries), numpy.ma.masked_where(Fseries<=0, Fseries), color=color_F, label='$Fatality$', zorder=6)
        if(any(Eseries) and plot_E=='line'):
            ax.plot(numpy.ma.masked_where(Eseries<=0, self.tseries), numpy.ma.masked_where(Eseries<=0, Eseries), color=color_E, label='$Exposed$', zorder=6)
        if(combine_D and (any(Dseries) and plot_D_E=='line' and plot_D_E=='line')):
            ax.plot(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, Dseries), color=color_D_E, label='$Detected$', zorder=6)
        else:
        ...
        if(any(Iseries) and plot_I=='line'):
            ax.plot(numpy.ma.masked_where(Iseries<=0, self.tseries), numpy.ma.masked_where(Iseries<=0, Iseries), color=color_I, label='$Infected$', zorder=6)
        ...
        if(legend):
            ...
            ax.legend(legend_handles[::-1], legend_labels[::-1], loc='upper right', facecolor='white', edgecolor='none', framealpha=0.9, prop={'size': "x-large"})
        if(title):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None, plot_S='line', plot_E='line', plot_I_pre='line', plot_I_S='line', plot_I_A='line',
        ...
        Fseries     = float(self.numF)/self.numNodes if plot_percentages else self.numF
        Dseries     = float(self.total_num_detected())/self.numNodes if plot_percentages else self.total_num_detected()
        D_Eseries   = float(self.numD_E)/self.numNodes if plot_percentages else self.numD_E 
        D_preseries = float(self.numD_pre)/self.numNodes if plot_percentages else self.numD_pre 
        D_Aseries   = float(self.numD_A)/self.numNodes if plot_percentages else self.numD_A 
        D_Sseries   = float(self.numD_S)/self.numNodes if plot_percentages else self.numD_S 
        Hseries     = float(self.numH)/self.numNodes if plot_percentages else self.numH 
        Eseries     = float(self.numE)/self.numNodes if plot_percentages else self.numE
        I_preseries = float(self.numI_pre)/self.numNodes if plot_percentages else self.numI_pre 
        I_Sseries   = float(self.numI_S)/self.numNodes if plot_percentages else self.numI_S
        I_Aseries   = float(self.numI_A)/self.numNodes if plot_percentages else self.numI_A
        Rseries     = float(self.numR)/self.numNodes if plot_percentages else self.numR 
        Sseries     = float(self.numS)/self.numNodes if plot_percentages else self.numS
        ...
        if(combine_D and any(Dseries) and plot_D_E=='stacked' and plot_D_pre=='stacked' and plot_D_S=='stacked' and plot_D_A=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, topstack+Dseries), topstack, color=color_D_S, alpha=0.5, label='$Detected$', zorder=2)
        ...
        if(combine_D and (any(Dseries) and plot_D_E=='shaded' and plot_D_pre=='shaded') and plot_D_S=='shaded' and plot_D_A=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, Dseries), 0, color=color_D_S, alpha=0.5, label='$Detected$', zorder=4)
        ...
        if(combine_D and (any(Dseries) and plot_D_E=='line' and plot_D_pre=='line' and plot_D_S=='line' and plot_D_A=='line')):
            ax.plot(numpy.ma.masked_where(Dseries<=0, self.tseries), numpy.ma.masked_where(Dseries<=0, Dseries), color=color_D_S, label='$Detected$', zorder=6)
        else:
            if(any(D_Eseries) and plot_D_E=='line'):
                ax.plot(numpy.ma.masked_where(D_Eseries<=0, self.tseries), numpy.ma.masked_where(D_Eseries<=0, D_Eseries), color=color_D_E, label='$D_E$', zorder=6)
