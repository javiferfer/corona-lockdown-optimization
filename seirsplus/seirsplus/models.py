...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_epoch(self, runtime, dt=0.1):
        ...
        t_eval    = numpy.arange(start=self.t, stop=self.t+runtime+0.001, step=dt)
        ...
        solution        = scipy.integrate.solve_ivp(lambda t, X: SEIRSModel.system_dfes(t, X, self.beta, self.sigma, self.gamma, self.xi, self.mu_I, self.mu_0, self.nu,
                                                        ...
                                                        t_span=[self.t, self.tmax+0.001], y0=init_cond, t_eval=t_eval
                                                   )
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None,  plot_S='line', plot_E='line', plot_I='line',plot_R='line', plot_F='line',
        ...
        Dseries     = float(self.numD_E+self.numD_I)/self.numNodes if plot_percentages else (self.numD_E+self.numD_I)
        D_Eseries   = float(self.numD_E)/self.numNodes if plot_percentages else self.numD_E
        D_Iseries   = float(self.numD_I)/self.numNodes if plot_percentages else self.numD_I
        Iseries     = float(self.numI)/self.numNodes if plot_percentages else self.numI
